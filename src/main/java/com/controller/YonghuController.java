...
//                            yonghuEntity.setCreateTime(date);//时间
    ...
    */
    ...
    public R login(String username, String password, String captcha, HttpServletRequest request) {
        ...
        // 检查用户状态
        if(yonghu.getYonghuStatus() == null || yonghu.getYonghuStatus() != 1)
            return R.error("您的账号尚未通过审核或已被拒绝，请联系管理员");
...
//    	ValidatorUtils.validateEntity(user);
        ...
        yonghu.setCreateTime(new Date());
        // 设置用户状态为待审核
        yonghu.setYonghuStatus(0);
        //注册区块链账号
        String address = weBASEService.registerChainCount(yonghu.getUsername());
        yonghu.setChainCount(address);
        yonghuService.insert(yonghu);
        Map map = new HashMap();
        map.put("chain_count",address);
        return R.ok(map);
    }
     ...
     * 审批用户注册申请
     */
    @RequestMapping(value = "/approveRegister")
    public R approveRegister(@RequestParam Map<String, Object> params, HttpServletRequest request){
        logger.debug("approveRegister方法:,,Controller:{},,params:{}",this.getClass().getName(),JSONObject.toJSONString(params));
        
        String role = String.valueOf(request.getSession().getAttribute("role"));
        if(StringUtil.isEmpty(role) || !"管理员".equals(role))
            return R.error(511,"只有管理员才能审批注册申请");
        
        Integer id = Integer.valueOf(String.valueOf(params.get("id")));
        Integer status = Integer.valueOf(String.valueOf(params.get("status"))); // 1-批准，2-拒绝
        
        if(id == null || status == null || (status != 1 && status != 2))
            return R.error(511,"参数错误");
        
        YonghuEntity yonghu = yonghuService.selectById(id);
        if(yonghu == null)
            return R.error(511,"用户不存在");
        
        yonghu.setYonghuStatus(status);
